...
class ConnectionWorker(QThread):
    ...
    def _handle_connection(self, task):
        ...
        success = self.connection_manager.connect_device(
            device_name, 
            task['password'],
    ...
    def _handle_deployment(self, task):
        """배포 처리"""
        device_name = task['device_name']
        success, output = self.connection_manager.deploy_config(
            device_name,
            task['commands']
        )
        self.command_result.emit(device_name, success, output)
...
class DeviceManagerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.deployment_manager = DeploymentManager(self.connection_manager)
        
        # 워커 스레드
        self.worker = ConnectionWorker(self.connection_manager)
    ...
    def _setup_device_tab(self):
        ...
        self.device_table.horizontalHeader().setSectionResizeMode(6, QHeaderView.Stretch)
    ...
    def _load_devices(self):
        ...
        for device in self.connection_manager.device_list:
    ...
    def _update_combo_boxes(self):
        ...
        for device in self.connection_manager.device_list:
            if self.connection_manager.is_connected(device.name):
    ...
    def _add_device(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if self.connection_manager.add_device(device):
                self._load_devices()
    ...
    def _remove_device(self):
        ...
        if reply == QMessageBox.Yes:
            for device_name in selected_devices:
                self.connection_manager.remove_device(device_name)
    ...
    def _backup_selected(self):
        ...
        for device_name in selected_devices:
            if self.connection_manager.is_connected(device_name):
    ...
    def _backup_all(self):
        """모든 장비 백업"""
        results = self.connection_manager.backup_all_devices()
    ...
    def _update_device_status(self):
        ...
        for row in range(self.device_table.rowCount()):
            device_name = self.device_table.item(row, 1).text()
            status = self.connection_manager.get_device_status(device_name)
